	...
	def remove_paid_invoice(self):
		...
		paid_invoices = frappe.db.sql("""
			...
			WHERE 
				...
				AND (
					si.status = 'paid'
					OR si.outstanding_amount = 0
				)
				AND docstatus = 1
		""", (report_date), as_dict=True)

		invoices_dict = {row.name: True for row in paid_invoices}
		...
		for row in self.data:
			...
			if voucher_type == 'Sales Invoice' and voucher_no in invoices_dict:
				continue
